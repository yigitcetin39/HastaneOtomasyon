...
public class LoginGUI extends JFrame {
	...
	public LoginGUI() {
		...
		JLabel lblNewLabel = new JLabel("Hastane Yönetim Sistemine Hos geldiniz");
		...
		JButton btn_hastaLogin = new JButton("Giris  Yap"); //Hasta giri� butonu
		btn_hastaLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(fld_hastaTc.getText().length()==0 || fld_hastaPass.getText().length()==0) {
					...
					if(key) {
						Helper.showMsg("Böyle bir hasta bulunamad� lüfen kayıt olunuz !");
					}
				}
			}
		});
		...
		JLabel lblNewLabel_2_1 = new JLabel("sifreniz :");
		...
		JButton btn_doctorLogin = new JButton("Giris Yap"); //doktor giri� butonu
			...
			public void actionPerformed(ActionEvent e) {
				if(fld_doctorTc.getText().length()==0 || fld_doctorPass.getText().length() == 0) {
					...
					try {
						...
						ResultSet rs = st.executeQuery("SELECT * FROM user");
						while(rs.next()) {
							if(fld_doctorTc.getText().equals(rs.getString("tcno")) && fld_doctorPass.getText().equals(rs.getString("password"))) { //e�er giri� yapan doktorun r�tbesi ba�hekim ise
								...
								if(rs.getString("type").equals("doktor")) {//e�er giri� yapan ki�inin r�tbesi doktor ise
